...
config.General.workArea = 'TriggerTagPerL1_%s' % production_tag
...
config.JobType.psetName = './run_trigger_tagPerL1_cfg.py'  ##########################
  ...
  with open(args.yaml) as f:
    ...
    for sample, info in doc['samples'].items():
      ...
      for part in parts:
        ...
        #config.JobType.outputFiles = ['_'.join(['TriggerL1', 'mc' if isMC else 'data', production_tag])+'.root']
        config.JobType.outputFiles = ['TriggerTagPerL1.root']
