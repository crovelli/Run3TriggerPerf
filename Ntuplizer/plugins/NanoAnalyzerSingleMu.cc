...
#ifndef CMSSW12plus
// Run 1 and 2
class NanoAnalyzerSingleMu : public edm::EDAnalyzer
#else
// Run 3
class NanoAnalyzerSingleMu : public edm::one::EDAnalyzer<>
#endif
{
public:
  explicit NanoAnalyzerSingleMu(const edm::ParameterSet&);
  ~NanoAnalyzerSingleMu();
...
NanoAnalyzerSingleMu::NanoAnalyzerSingleMu(const edm::ParameterSet& iConfig)
{

  //std::cout << "DC: NanoAnalyzerSingleMu" << std::endl;
...
NanoAnalyzerSingleMu::~NanoAnalyzerSingleMu()
{
...
void
NanoAnalyzerSingleMu::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (unsigned int i = 0, n = HLTtriggers_->size(); i < n; ++i) {
    ...
    if(trigNames.triggerName(i).find("IP6")!= std::string::npos){
    ...
    if(trigNames.triggerName(i).find("eta1p22_mMax6")!= std::string::npos){
      ...
      if(HLTtriggers_->accept(i)){
	isTriggeredELE = true;
	finalTriggerNameELE=trigNames.triggerName(i);  
  ...
  //if(!isTriggeredELE) return;
  
  //std::cout<<"!!!!! single event this fired"<<finalTriggerName<<std::endl;
  //std::cout<<"!!!!! single event this fired"<<finalTriggerNameELE<<std::endl;
...
////
      for (unsigned h = 0, n = pathNamesAll.size(); h < n; ++h) {
      	if(pathNamesAll[h]==finalTriggerNameELE) isPathExist = true;
      }
////
...
////	//	std::cout << "Muon" << imuon << " matches to the trigger object dR = " << trigger_dR << ", obj pT = " << obj.pt() << ", muon pT = " << muon.pt() << " " << obj.pdgId() << std::endl;
    ...
    //std::cout<<"!!!!!! trigmatch wooo electrons"<<std::endl;
...
void NanoAnalyzerSingleMu::beginJob(const edm::ParameterSet& iConfig)
{
...
void NanoAnalyzerSingleMu::beginRun(const edm::Run &iRun, const edm::EventSetup &iStp)
{
...
void NanoAnalyzerSingleMu::fillDescriptions(edm::ConfigurationDescriptions & descriptions) 
{
  //std::cout << "fillDescriptions!" << std::endl;
}

// ------------ method called when ending the processing of a run  ------------  Qun below
void NanoAnalyzerSingleMu::endRun(edm::Run const&, edm::EventSetup const&)
{
...
// branch title creation
void NanoAnalyzerSingleMu::createBranch() { 
...
void NanoAnalyzerSingleMu::reset(void){
...
DEFINE_FWK_MODULE(NanoAnalyzerSingleMu);
