...
NanoAnalyzerDoubleMu::NanoAnalyzerDoubleMu(const edm::ParameterSet& iConfig)
{
  ...
  std::cout << "DC: NanoAnalyzerDoubleMu" << std::endl;
...
void
NanoAnalyzerDoubleMu::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
...
/*
  for(size_t imuon = 0; imuon < muons_->size(); imuon++){
    ...
    for (pat::TriggerObjectStandAlone obj : *triggerObjects) {
      ...
      Float_t trigger_dR = reco::deltaR(obj.eta(), obj.phi(),	muon.eta(), muon.phi());
...
//  fMagneticField = fieldHandle.product();
  ...
  reco::VertexCollection::const_iterator vtx = vertices_->begin();
